package  {
	...
	public class MethylAldehyde extends MovieClip implements IQuestion{
		
		private var hitSlot:Slot;
		
		public function MethylAldehyde(x:Number, y:Number) {
		this.x=x;
		this.y=y;
		// constructor code
		}
		...
		public function isLegalDrop(dropObj:MovieClip):Boolean
		{
			...
			else if(slot_O.hitTestObject(dropObj))
			{
				...
				{
					slot_O.setElectronVisible(true,true,true,true,false,false,true,true);
					slot_O.setElectronColor(Color.PURPLE);
					hitSlot = slot_O;
		...
		{
			if(!slot_C.isAvailable()&&!slot_O.isAvailable()&&!slot_H1.isAvailable()&&!slot_H2.isAvailable())
			{
